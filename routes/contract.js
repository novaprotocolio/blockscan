...
var versions = JSON.parse(
  fs.readFileSync('./utils/solc-bin/bin/list.json')
).builds.reverse();
...
router.post('/verify', function(req, res, next) {
  ...
  async.waterfall(
    [
      function(callback) {
        web3.eth.subscribe(
          { fromBlock: '0x00', toAddress: [contractAddress] },
          function(err, traces) {
      ...
      function(traces, callback) {
        ...
        traces.forEach(function(trace) {
          if (
            trace.type === 'create' &&
            trace.result.address === contractAddress &&
            !trace.error
          ) {
      ...
      function(creationBytecode, callback) {
        ...
        var solcCommand =
          '/usr/bin/nodejs ./utils/compile.js ' + tmpName + ' ' + outputName;
        ...
        exec(solcCommand, function(error, stdout, stderr) {
          ...
          if (error || !stdout) {
            ...
            callback(
              'Compiler is currently unavailable. Please try again later...',
              null
            );
            return;
          }
          ...
          try {
            ...
            callback(
              'An unexpected error occurred during compilation, please try again later...',
              null
            );
            return;
          }
          ...
          // Remove swarm hash
          var blockchainBytecodeClean = creationBytecode.replace(
            /a165627a7a72305820.{64}0029/gi,
            ''
          );
          var contractBytecodeClean = contractBytecode.replace(
            /a165627a7a72305820.{64}0029/gi,
            ''
          );
          ...
          if (blockchainBytecodeClean.indexOf(contractBytecodeClean) === 0) {
            constructorArgs = blockchainBytecodeClean.replace(
              contractBytecodeClean,
              ''
            );
            blockchainBytecodeClean = blockchainBytecodeClean.replace(
              constructorArgs,
              ''
            );
          }
          ...
          if (contractBytecodeClean !== blockchainBytecodeClean) {
            var similarity = stringSimilarity.compareTwoStrings(
              contractBytecodeClean,
              blockchainBytecodeClean
            );
            var errorStr =
              'Unable to verify contract (Similarity: ' +
              similarity +
              ') \nGot: ' +
              contractBytecodeClean +
              '\n\nExpected: ' +
